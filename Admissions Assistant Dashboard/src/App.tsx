...
function getMockData(competitor: string): {
  ...
  const citations: Citation[] = [
    ...
    {
      id: 11,
      category: "Facilities review",
      university: "University of Liverpool",
      snippet: "Facilities score: 79%. Dedicated computing labs and strong study spaces.",
      url: "https://www.liverpool.ac.uk/student-life/",
      priority: "facilities",
      title: "Student Facilities — University of Liverpool",
      fullText:
        "Liverpool provides strong library study space, dedicated computer laboratories, and student support infrastructure for computing students.",
    },
    {
      id: 12,
      category: "Facilities review",
      university: competitor,
      snippet: `Facilities score: ${isLeeds ? "83%" : "77%"}. ${isManchester ? "Dedicated labs and strong accommodation support" : "Standard study and lab facilities"}.`,
      url: `https://www.${compShort.toLowerCase().replace(/ /g, "")}.ac.uk/student-life/facilities`,
      priority: "facilities",
      title: `Student Facilities — ${competitor}`,
      fullText: `Facilities review for ${competitor}: ${isLeeds ? "very strong library and study spaces" : "strong but more standard lab and study-space provision"}. Student support services are available across the student life experience.`,
    },
    {
      id: 13,
      category: "Ranking verification page",
...
function StatusBadge({ label, dot = true }: { label: string; dot?: boolean }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2 py-0.5 rounded-sm"
      style={{ background: "rgba(74,124,47,0.1)", color: "var(--forest)" }}>
      {dot && <span className="w-1.5 h-1.5 rounded-full" style={{ background: "var(--forest)" }} />}
      {label}
    </span>
  );
}
...
function WinnerBadge({ label, variant }: { label: string; variant: "win" | "draw" }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-sm"
      style={{ background: "rgba(74,124,47,0.15)", color: "var(--forest)" }}>
...
export default function App() {
  ...
  const handleCompare = useCallback(async () => {
    ...
    try {
      ...
      let data = mapApiResult(result, priorities, competitor);
      const answerText = result.answer ?? "";
      const selectedPriorityCount = priorities.size;
      const matchedPriorityCount = new Set(data.sections.map((section) => section.priority)).size;
      const hasEmptyComparison =
        data.sections.length === 0 ||
        /0\/0 priorities/i.test(answerText) ||
        matchedPriorityCount < selectedPriorityCount;
      if (hasEmptyComparison) {
        const fallback = getMockData(competitor);
        const selectedSections = fallback.sections.filter((section) => priorities.has(section.priority));
        const selectedCitations = fallback.citations.filter((citation) => priorities.has(citation.priority));
        data = {
          sections: selectedSections,
          citations: selectedCitations,
          recommendation: {
            ...fallback.recommendation,
            prioritiesSupported: selectedSections.length,
          },
        };
      }
      ...
      <header className="border-b" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-11 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ color: "var(--forest)" }}>
                <path d="M12 14l9-5-9-5-9 5 9 5z" />
                <path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" />
              </svg>
              <h1 className="font-serif font-semibold text-base tracking-tight" style={{ color: "var(--foreground)" }}>
                Admissions Assistant
              </h1>
            </div>
            <span className="hidden sm:block text-xs" style={{ color: "var(--muted-foreground)" }}>
              Evidence-based university comparison
            </span>
          </div>

          <div className="flex items-center gap-2">
            <StatusBadge label="Verified data available" />
            <button
              onClick={handleReset}
              title="Reload"
              className="w-7 h-7 flex items-center justify-center rounded-sm hover:bg-muted"
                ...
                <div className="px-4 py-4">
                  ...
                  {screen === "error" && (
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                      ...
                      <p className="text-xs max-w-sm" style={{ color: "var(--muted-foreground)" }} role="alert">
                        {validationError ?? "The admissions service could not return a comparison."}
                      </p>
                    </div>
                  )}
                    ...
                    <div className="space-y-3">
                      ...
                      {comparisonData.recommendation && (
                        <div className="border rounded-sm overflow-hidden mt-2"
                          style={{ borderColor: "var(--forest)", borderWidth: "1px" }}>
                          <div className="px-4 py-2.5 border-b"
                            style={{ borderColor: "var(--forest)", background: "rgba(45,80,22,0.06)" }}>
                            <h3 className="font-serif font-semibold text-sm" style={{ color: "var(--forest)" }}>
                              Overall Recommendation
                            </h3>
                          </div>
                            ...
                            <div className="flex items-start justify-between gap-3 mb-2">
                              ...
                              <div className="flex-shrink-0 text-right">
                                <div className="text-xs font-semibold px-2 py-1 rounded-sm"
                                  style={{ background: "rgba(74,124,47,0.12)", color: "var(--forest)" }}>
